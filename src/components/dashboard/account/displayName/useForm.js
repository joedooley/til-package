...
const schema = yup.object().shape({
  displayName: yup.string().required().min(2).max(32).trim(),
});

export default function useDisplayNameForm() {
  ...
  React.useEffect(() => {
    if (formState.isSubmitSuccessful && !errors?.displayName) {
      reset({ ...submittedData });
    }
  ...
  const onSubmit = React.useCallback(
    payload => {
      return client('/api/user/update', { body: { data: payload } })
        ...
        .catch(error => setError('displayName', { type: 'server', message: error.message }));
    },
